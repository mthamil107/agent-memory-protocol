...
def _add_common_flags(sub: argparse.ArgumentParser) -> None:
    ...
    sub.add_argument(
        ...
        help="agent_id scope (default: memorywire-cli).",
    )
    sub.add_argument(
        "--store",
        dest="stores",
        action="append",
        default=None,
        help="Store URL. Repeat for multi-store recall (default: sqlite-vec://./memorywire-cli.db).",
    )
...
async def _handle_remember(args: argparse.Namespace) -> int:
    """Implement ``memorywire remember <content>``."""
...
async def _handle_recall(args: argparse.Namespace) -> int:
    """Implement ``memorywire recall <query>``."""
...
def main(argv: Sequence[str] | None = None) -> int:
    """Run the ``memorywire`` CLI. Returns the process exit code."""
    ...
    if handler is None:
        ...
        print(f"memorywire: unknown command: {args.command}", file=sys.stderr)
        return 1
    ...
    try:
        ...
        print(f"memorywire: backend error: {exc}", file=sys.stderr)
        return 2
