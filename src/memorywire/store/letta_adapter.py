"""Letta backend adapter for memorywire (Phase 3).

This module exposes :class:`LettaStore`, a :class:`memorywire.store.MemoryStore`
implementation that translates memorywire operations onto the official
...
# memorywire fields encoded as Letta tags. ``key:value`` form keeps the tag set a
# flat list[str] (Letta's only supported shape) while still letting the
# recall path parse the original memorywire fields back out. Choose short prefixes
...
def _tags_to_amp_overlay(tags: list[str] | None) -> dict[str, Any]:
    """Parse an memorywire-tag list back into a flat overlay dict.

    Inverse of :func:`_amp_overlay_to_tags`. Unrecognised tags are
    ignored. The returned dict carries the original memorywire-field names
...
def _amp_overlay_to_tags(
    *,
    ...
    """Encode the memorywire overlay onto a flat tag list for Letta.
...
class LettaStore:
    """memorywire adapter for the official ``letta-client`` SDK.
    ...
    def from_url(cls, url: str, *, client: Any | None = None) -> LettaStore:
        ...
        spec-gap: agent_id is *required* at the Letta layer but memorywire's
    ...
    async def remember(self, req: RememberRequest) -> RememberResponse:
        ...
        memorywire fields Letta doesn't natively model (``type``, ``confidence``,
        ...
        server may split content into multiple passages on chunking). memorywire
        ...
        # passages so the memorywire response shape stays valid.
        if first_id is None:
    ...
    async def recall(self, req: RecallRequest) -> RecallResponse:
        """Retrieve passages via ``agents.passages.search`` and map to memorywire hits.
    ...
    async def forget(self, req: ForgetRequest) -> ForgetResponse:
        ...
        if req.filter:
            ...
            for record in records:
                ...
                # the passage dict and the parsed memorywire overlay/metadata.
                matched = True
