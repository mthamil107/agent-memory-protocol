"""mem0 backend adapter for memorywire (Phase 3).

This module exposes :class:`Mem0Store`, a :class:`memorywire.store.MemoryStore`
implementation that translates memorywire operations onto the public ``mem0``
...
# memorywire fields that mem0 does not understand natively are flattened into
...
class Mem0Store:
    """memorywire adapter for the ``mem0`` Python SDK.
    ...
    def _record_matches_filter(record: dict[str, Any], flt: dict[str, Any]) -> bool:
        """Apply a flat memorywire filter dict to a mem0 record returned by ``get_all``.
    ...
    async def remember(self, req: RememberRequest) -> RememberResponse:
        ...
        memorywire fields mem0 doesn't natively support (``type``, ``confidence``,
        ...
        (returns ``{"results": [...]}``). memorywire v0 only surfaces the *first*
        ...
        if principal is None:
            # mem0 itself would raise here, but the memorywire error is friendlier.
            raise ValueError("mem0 adapter requires either `user_id` or `agent_id` on the request")

        # Merge caller-supplied metadata with the memorywire-specific overlay.
        ...
        # memorywire response shape stays valid; downstream audit can detect the
    ...
    async def recall(self, req: RecallRequest) -> RecallResponse:
        """Retrieve memories via ``mem0.search`` and map to memorywire hits.
        ...
        # mem0 uses ``top_k`` (default 20); memorywire uses ``k`` (default per spec
        ...
        # behaviour at the memorywire-router layer; mem0's internal pipeline is
