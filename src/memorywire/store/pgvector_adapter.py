...
* The default embedder mirrors :mod:`memorywire.store.sqlite_vec`: 384-d output from
...
class PgVectorStore:
    """memorywire adapter for PostgreSQL + ``pgvector``.
    ...
    schema:
        Postgres schema namespace under which memorywire tables live. Defaults to
        ``"memorywire"``. Must match ``[A-Za-z_][A-Za-z0-9_]*``.
    embedding_dim:
        Vector length the embedder produces. Defaults to 384 to match
        sentence-transformers' all-MiniLM-L6-v2 (same as
        :mod:`memorywire.store.sqlite_vec`).
    embedder:
    ...
    async def _resolve_entity(self, conn: Any, agent_id: str, key: str) -> list[Any]:
        ...
        Mirrors :class:`memorywire.store.sqlite_vec.SqliteVecStore._resolve_entity`.
        """
        rows = await conn.fetch(
