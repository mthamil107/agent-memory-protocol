# memorywire and MCP Ã¢â‚¬â€ Composing, Not Competing

> Version: v0 Ã¢â‚¬â€ 2026-05-27
> Audience: prospective adopters, MCP working group, paper reviewers.
> One-line summary: memorywire is what MCP would be if MCP had a memory
...
protocol entirely. memorywire exists to give memory the same first-class
treatment MCP gave tool-use, and to keep iterating on that design at
the speed an unblocked draft allows. We expect to propose memorywire as an
...
itself remains memory-blind. memorywire is the layer that names memory as
its own thing.
...
| Concern | MCP today | memorywire v0 |
|---|---|---|
...
Where the rows show "first-class" on the memorywire side and "none" on the
MCP side, that is the gap memorywire fills. Where the rows show "yes" on
the MCP side and "not the focus" on the memorywire side, memorywire defers Ã¢â‚¬â€ we
...
### 4.1 memorywire-as-MCP-tool (works today)

The simplest composition. Wrap an memorywire `Memory` facade as an MCP
server that exposes five tools Ã¢â‚¬â€ one per operation Ã¢â‚¬â€ and route the
JSON-Schema-validated payload straight into the memorywire request models:
...
Pros: zero new infrastructure; any MCP-aware agent can use memorywire
...
### 4.2 memorywire-as-MCP-resource (works today)
...
### 4.3 memorywire-as-MCP-extension (proposed, v0.5 timeline)

The future state. memorywire becomes a published MCP extension Ã¢â‚¬â€
...
breaking changes. The memorywire spec explicitly reserves the right to
...
needs a spec document that stands on its own. Designing memorywire as a
...
- **v0.2 (Aug 2026)** Ã¢â‚¬â€ File an MCP RFC proposing memory extensions;
  cite memorywire as a candidate reference implementation. Begin
...
- **v0.5 (Q3 2026)** Ã¢â‚¬â€ Submit a memory-extension proposal to the
  ...
  submit memorywire v0.5 as an IETF Internet-Draft
  (`draft-<name>-memorywire-00`).
...
v0.5: keep memorywire's JSON-Schema 2020-12 definitions translatable to
MCP's JSON-RPC type system; keep operation names and field shapes
identifier-clean; avoid memorywire-specific transport assumptions in the
...
will be whatever the working group decides; memorywire's role is to bring
...
memorywire is not an alternative to MCP. The two protocols address adjacent
problems Ã¢â‚¬â€ tool-use (MCP) and memory (memorywire) Ã¢â‚¬â€ and the agents we
build use both at the same time. A realistic agent loop looks like
this:

```python
# Tool-use over MCP, memory over memorywire, in one agent loop
async def step(user_message: str):
    # Recall relevant memory via memorywire
    ...
    # If the model produced something worth remembering, write via memorywire
...
invocation, memorywire handles memory operations. Adopting memorywire does not
mean dropping MCP. Adopting MCP does not preclude adopting memorywire. The
acknowledgment line in the memorywire README is sincere: memorywire is *modelled
...
- **JSON-RPC vs REST shape.** memorywire's schemas are written against
  ...
  there is real design space in how to express memorywire operations as
...
- **Core vs extension.** Should memory live in core MCP, as one of
  ...
  ours.
- **Mapping the type taxonomy.** memorywire's four-type taxonomy
  ...
  Unclear.
- **Governance plane.** The memorywire governance channel (diff-and-
...
primitive should live in MCP, the memorywire spec and reference
implementation are intended as input material. Spec is at
[`docs/spec/v0.md`](spec/v0.md); reference impl is in `src/memorywire/`.
...
- If you already run an MCP server and want memory now: Ã‚Â§4.1
  (memorywire-as-MCP-tool). Fifteen-minute integration, no new client
  required.
- If your agent reads from resources rather than calls tools:
  Ã‚Â§4.2 (memorywire-as-MCP-resource).
- If you can wait until v0.5 for tighter integration: track this
  doc and the MCP RFC; the extension path lands then.

**Paper reviewers positioning memorywire against MCP in related work.**
memorywire and MCP are non-overlapping in their primitives. The honest
positioning is "memorywire is to memory what MCP is to tool-use; the two
compose in production agents." If you need a citation for that
framing, this document and [`docs/spec/v0.md`](spec/v0.md) Ã‚Â§1
("memorywire is to agent memory what MCP is to agent tool-use") are the
references.
...
- memorywire wire-format spec v0 Ã¢â‚¬â€ [`docs/spec/v0.md`](spec/v0.md)
- memorywire roadmap and standards-engagement plan Ã¢â‚¬â€
  [`docs/kickoff/FUTURE.md`](kickoff/FUTURE.md) (see
  "Standards-body engagement strategy")
- Original memorywire kickoff draft Ã¢â‚¬â€
  [`docs/kickoff/memorywire-SPEC-v0.md`](kickoff/memorywire-SPEC-v0.md)
- memorywire architecture, RRF semantics, FSM format Ã¢â‚¬â€
