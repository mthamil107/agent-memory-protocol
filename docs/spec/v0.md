# memorywire Ã¢â‚¬â€ Specification v0

**Version:** 0.1.0-draft &nbsp;Ã‚Â·&nbsp; **Status:** draft v0.1.0 Ã¢â‚¬â€ subject to change &nbsp;Ã‚Â·&nbsp; **Date:** 2026-05-26

> memorywire is the wire-format specification for agent memory operations. It defines the operations, memory types, and data shapes exchanged between memory clients, the memory router, and backend memory stores. The reference implementation lives in [`src/memorywire/`](../../src/memorywire/). Schemas are written against JSON Schema Draft 2020-12.
>
> Context for the design choices in this document:
> - [`docs/kickoff/FINDINGS-CONTEXT.md`](../kickoff/FINDINGS-CONTEXT.md) Ã¢â‚¬â€ why this protocol exists
> - [`docs/kickoff/ARCHITECTURE.md`](../kickoff/ARCHITECTURE.md) Ã¢â‚¬â€ storage shape, RRF semantics, FSM format
> - [`docs/kickoff/memorywire-SPEC-v0.md`](../kickoff/memorywire-SPEC-v0.md) Ã¢â‚¬â€ original draft this document refines
...
1. Five operations Ã¢â‚¬â€ `remember`, `recall`, `forget`, `merge`, `expire` Ã¢â‚¬â€ with stable JSON schemas.
...
All timestamp fields (`stored_at`, `created_at`, `updated_at`, `expires_at`, `reviewed_at`) are Unix epoch milliseconds as integers, per [`ARCHITECTURE.md` Ã‚Â§3](../kickoff/ARCHITECTURE.md).
...
> Full schema: [src/memorywire/schemas/operations/remember.json](../../src/memorywire/schemas/operations/remember.json)
...
If `approval_required` is true (or governance policy forces it), the response sets `pending_approval: true` and `approval_url` points to the governance UI. The memory is staged but not committed until approval Ã¢â‚¬â€ `stores` is the empty array while pending.
...
> Full schema: [src/memorywire/schemas/operations/recall.json](../../src/memorywire/schemas/operations/recall.json)
...
`supporting` lists related memory ids that contributed to the score via graph-hop boost (empty when `hops=0`). `score` magnitude depends on the fusion algorithm Ã¢â‚¬â€ see Ã‚Â§5.
...
> Full schema: [src/memorywire/schemas/operations/forget.json](../../src/memorywire/schemas/operations/forget.json)
...
> Full schema: [src/memorywire/schemas/operations/merge.json](../../src/memorywire/schemas/operations/merge.json)

> **Editor's note:** the kickoff draft defines no response shape for `merge` and no per-strategy semantics. v0.1 sketches the response below as guidance Ã¢â‚¬â€ the schema for it is not authored at v0 (deferred to v0.2 once adapters return concrete per-store aggregates):
>
> ```json
> {
>   "canonical":     "01HZK...",
...
> - `keep_canonical` Ã¢â‚¬â€ canonical row is preserved verbatim; duplicates are dropped after pointer migration.
> - `merge_content` Ã¢â‚¬â€ backends MUST union metadata keys (last-write-wins on conflict, ordered by `created_at` ascending) and MAY concatenate `content` with a newline separator. The merged row's `confidence` is `max(confidence)` over all merged rows.
> - `keep_highest_confidence` Ã¢â‚¬â€ whichever row has the highest `confidence` becomes the new canonical content; ties are broken by older `created_at`.
...
> Full schema: [src/memorywire/schemas/operations/expire.json](../../src/memorywire/schemas/operations/expire.json)

> **Editor's note:** the kickoff draft defines no response shape for `expire`, nor behaviour for backends lacking last-recalled-at tracking. v0.1 sketches the response below as guidance Ã¢â‚¬â€ the schema for it is not authored at v0 (deferred to v0.2):
>
> ```json
> {
>   "matched_count": 412,
...
`capabilities` is a set of strings declaring what the backend supports Ã¢â‚¬â€ e.g. `{"semantic", "episodic", "fts", "vector", "graph", "procedural", "recall_tracking"}`. The router uses this to skip stores that do not support a given operation.
...
- **`remember`** Ã¢â‚¬â€ fans out to all child stores by default; `policy: "primary_only"` writes only to the first.
- **`recall`** Ã¢â‚¬â€ fans out to all stores in parallel, fuses results per `fusion`.
- **`forget`** / **`merge`** / **`expire`** Ã¢â‚¬â€ fans out to all stores; aggregates per-store responses.

Fusion algorithms:

- **`rrf`** (default) Ã¢â‚¬â€ Reciprocal Rank Fusion, k=60:

  ```
  score(item) = ÃŽÂ£ (1 / (60 + rank_i(item)))   for each store i
  ```

- **`max`** Ã¢â‚¬â€ take the highest score per item across stores.
- **`weighted`** Ã¢â‚¬â€ per-store weights from router config; sum weighted scores. Magnitudes are not normalized.
...
In `remember` requests, the FSM JSON is currently carried as a string in `content` (because `content` is required to be a string in v0). Promoting procedural memory to a structured `content` field is tracked for v0.2 Ã¢â‚¬â€ see Ã‚Â§11.
...
- v0.5 Ã¢â€ â€™ v1.0: stabilization. Operations and schemas frozen.
- v1.0 Ã¢â€ â€™ publish as IETF Internet-Draft for cross-language adoption.
...
- **Embedding metadata** Ã¢â‚¬â€ should `remember()` accept a pre-computed embedding, or always compute server-side?
- **Cross-agent sharing** Ã¢â‚¬â€ should memorywire define a `share` operation for inter-agent memory transfer? Likely v0.2.
- **Streaming `recall`** Ã¢â‚¬â€ `recall_stream()` for very large `k`? Defer to v1.
- **Privacy-intent flags** Ã¢â‚¬â€ the "Layer 3 gap" from Module 11. Should `remember()` carry `privacy_intent: {"retain": "30d", "share_with": ["agent-x"], "user_consent_id": "..."}`? Likely v0.2.
- **Federated memory** Ã¢â‚¬â€ multi-tenant scoping primitives. Defer to v1.
- **Structured `content` for procedural memory** Ã¢â‚¬â€ promote FSM payloads from a string blob in `content` to a typed field. Tracked for v0.2.
...
- [Model Context Protocol](https://modelcontextprotocol.io) Ã¢â‚¬â€ for the cross-vendor MCP-shaped pattern
- [OpenTelemetry semconv](https://opentelemetry.io/docs/specs/semconv/) Ã¢â‚¬â€ for stable identifier patterns
- [GenAI Semantic Conventions](https://opentelemetry.io/docs/specs/semconv/gen-ai/) Ã¢â‚¬â€ for cross-tool tracing
...
- LoCoMo benchmark
- BEAM benchmark
- "Remember Me, Refine Me" Ã¢â‚¬â€ procedural memory
- "Governed Memory" arXiv 2603.17787 Ã¢â‚¬â€ automated agent-memory governance (human-gated contrast)
...
- [`examples/remember/01_semantic_fact.json`](./examples/remember/01_semantic_fact.json) Ã¢â‚¬â€ a canonical semantic fact about a known user, written to a single store with high confidence.
- [`examples/remember/02_episodic_with_ttl.json`](./examples/remember/02_episodic_with_ttl.json) Ã¢â‚¬â€ edge case: an episodic event with `expires_at` TTL, no `user_id`, and an empty `metadata` object.
- [`examples/remember/03_procedural_fsm_with_approval.json`](./examples/remember/03_procedural_fsm_with_approval.json) Ã¢â‚¬â€ a procedural FSM written with `approval_required: true`, staged behind the governance channel.

### `recall`

- [`examples/recall/01_simple_topk.json`](./examples/recall/01_simple_topk.json) Ã¢â‚¬â€ natural-language query, default `k=5`, default RRF fusion, no type filter.
- [`examples/recall/02_multi_type_graph_hop.json`](./examples/recall/02_multi_type_graph_hop.json) Ã¢â‚¬â€ multi-type filter with `hops=1` graph boost and `fresher_than_days=30` recency window.
- [`examples/recall/03_weighted_cross_store.json`](./examples/recall/03_weighted_cross_store.json) Ã¢â‚¬â€ `fusion: "weighted"` across three heterogeneous stores with a custom `filter`.

### `forget`

- [`examples/forget/01_by_ids.json`](./examples/forget/01_by_ids.json) Ã¢â‚¬â€ soft delete a small batch of memories by explicit id list.
- [`examples/forget/02_by_filter.json`](./examples/forget/02_by_filter.json) Ã¢â‚¬â€ filter-only deletion scoped by `user_id` and `type`.
- [`examples/forget/03_hard_delete_with_reason.json`](./examples/forget/03_hard_delete_with_reason.json) Ã¢â‚¬â€ GDPR-style hard delete with audit `reason`.

### `merge`

- [`examples/merge/01_two_duplicates.json`](./examples/merge/01_two_duplicates.json) Ã¢â‚¬â€ two near-duplicates collapsed into one canonical entity with default `keep_canonical` strategy.
- [`examples/merge/02_three_way.json`](./examples/merge/02_three_way.json) Ã¢â‚¬â€ three-way merge using `keep_highest_confidence` and entity-name addressing.
- [`examples/merge/03_merge_content.json`](./examples/merge/03_merge_content.json) Ã¢â‚¬â€ `strategy: "merge_content"` unions metadata and concatenates content across rows.

### `expire`

- [`examples/expire/01_ttl_by_age.json`](./examples/expire/01_ttl_by_age.json) Ã¢â‚¬â€ TTL by age: expire `episodic` older than 90 days with default `action: "forget"`.
- [`examples/expire/02_no_recall_archive.json`](./examples/expire/02_no_recall_archive.json) Ã¢â‚¬â€ cold-storage policy: `no_recall_in_days: 180` with `action: "archive"`.
- [`examples/expire/03_low_confidence_demote.json`](./examples/expire/03_low_confidence_demote.json) Ã¢â‚¬â€ demote (not delete) stale low-confidence semantic facts.
