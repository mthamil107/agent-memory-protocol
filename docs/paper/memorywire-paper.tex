...
% --- USENIX template swap point ---------------------------------------------
% Replace this preamble block with the USENIX 2019_v3 boilerplate when
% finalizing. The current `article` preamble uses portable packages so the
% draft compiles standalone.
% ----------------------------------------------------------------------------
...
\author{Anonymous (double-blind review pending)}
\date{}
...
We state up front what this paper is \emph{not}. It is not a new algorithm: RRF is from Cormack et al.~\cite{cormack2009rrf}, the four-type taxonomy is from Tulving~\cite{tulving1972episodic} and Squire~\cite{squire1992declarative}, procedural FSMs use \texttt{pytransitions}, and STM/LTM consolidation predates the frameworks we cite --- the contribution is composition and standardization, not new substrate. It is not an LLM contribution: we do not train models, propose model-dependent heuristics, or run LongMemEval~\cite{wu2024longmemeval} / LoCoMo~\cite{maharana2024locomo} / BEAM in v0 (those require gated GPT-4-class graders and are the largest v0.2 deliverable per \Cref{sec:future}). And it is not a stabilization paper: memorywire v0 reserves the right to break wire-format compatibility through v0.5 (\Cref{sec:spec-versioning}); the IETF Internet-Draft and MCP-WG extension paths in \Cref{sec:related} and \Cref{sec:future} are intent, not commitment.
...
\textbf{Benchmark scope.} This paper does not report LongMemEval, LoCoMo, or BEAM. We say so up front because reviewers familiar with the agent-memory benchmark literature will look for those numbers first; they are not here in v0 by design (\Cref{sec:intro-limitations}) and are the primary v0.2 evaluation deliverable.
