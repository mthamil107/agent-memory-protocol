...
def _assert_safe_public_config(
    ...
    allow_unauth:
        Truthy when ``MEMORYWIRE_UI_ALLOW_UNAUTHENTICATED_PUBLIC=1`` is set.
    ...
    if is_loopback or has_token or allow_unauth:
        return
    print(
        "amp_ui: refusing to start a public-bound UI without MEMORYWIRE_UI_TOKEN.\n"
        f"MEMORYWIRE_UI_HOST is set to {host!r}. Either:\n"
        "  - Set MEMORYWIRE_UI_HOST=127.0.0.1 (loopback-only), OR\n"
        "  - Set MEMORYWIRE_UI_TOKEN=<a-bearer-token-string> via env or "
        "`fly secrets set`.\n"
        "See ui/README.md for token generation.\n"
        "To explicitly opt out of this check (e.g. a demo behind your "
        "own auth layer), set MEMORYWIRE_UI_ALLOW_UNAUTHENTICATED_PUBLIC=1.",
        file=sys.stderr,
    )
    raise SystemExit(1)
...
def main() -> None:
    ...
    host = os.environ.get("MEMORYWIRE_UI_HOST", "127.0.0.1")
    port = int(os.environ.get("MEMORYWIRE_UI_PORT", "8765"))
    agent_id = os.environ.get("MEMORYWIRE_UI_AGENT_ID", "default")
    db_path = os.environ.get("MEMORYWIRE_UI_DB_PATH")
    token = os.environ.get("MEMORYWIRE_UI_TOKEN") or None
    allow_unauth = os.environ.get("MEMORYWIRE_UI_ALLOW_UNAUTHENTICATED_PUBLIC", "") in {
        "1",
        "true",
        "yes",
        "on",
    ...
    try:
        ...
        print(f"[memorywire-ui] FATAL: {exc}", file=sys.stderr)
        sys.exit(1)
